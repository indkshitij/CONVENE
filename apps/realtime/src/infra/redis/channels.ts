...
// channel's name). Must match apps/api/src/infra/redis/channels.ts
// exactly (see that file's own comment — apps/* may not import apps/*).
// apps/api publishes (after its own Postgres commit); this gateway only
// ever subscribes and reads the replay buffer, never writes to it.
