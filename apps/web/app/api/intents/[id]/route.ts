...
export async function PATCH(
  ...
  const parsed = intentsValidation.updateIntentSchema.safeParse(await request.json());
  if (!parsed.success) return validationErrorResponse(parsed.error);

  try {
    const data = await apiFetch<IntentResponse>(`/intents/${id}`, {
      method: "PATCH",
...
export async function DELETE(
  ...
  try {
    await apiFetch(`/intents/${id}`, { method: "DELETE", accessToken: session.accessToken });
